...
; RUN: opt --load-pass-plugin=libjulia-codegen%shlibext -passes='JuliaFunctionSplitting,function(GCInvariantVerifier),verify' -julia-split-block-threshold=30 -julia-split-block-insts=16 -julia-split-region-insts=0 -S %s | FileCheck %s --check-prefix=CALLER
; RUN: opt --load-pass-plugin=libjulia-codegen%shlibext -passes='JuliaFunctionSplitting,function(GCInvariantVerifier),verify' -julia-split-block-threshold=30 -julia-split-block-insts=16 -julia-split-region-insts=0 -S %s | FileCheck %s --check-prefix=CALLEE
; RUN: opt --load-pass-plugin=libjulia-codegen%shlibext -passes='JuliaFunctionSplitting,function(GCInvariantVerifier),verify' -julia-split-block-threshold=30 -julia-split-block-insts=16 -julia-split-region-insts=0 -julia-split-direct-arg-limit=4 -S %s | FileCheck %s --check-prefix=SPILL
; RUN: opt --load-pass-plugin=libjulia-codegen%shlibext -passes='JuliaFunctionSplitting,function(GCInvariantVerifier,LateLowerGCFrame),verify' -julia-split-block-threshold=30 -julia-split-block-insts=16 -julia-split-region-insts=0 -S %s | FileCheck %s --check-prefix=LOWER
; RUN: opt --load-pass-plugin=libjulia-codegen%shlibext -passes='JuliaFunctionSplitting,function(GCInvariantVerifier),verify' -julia-split-function-threshold=50 -julia-split-block-insts=40 -julia-split-region-insts=0 -S %s | FileCheck %s --check-prefix=MULTI
; RUN: opt --load-pass-plugin=libjulia-codegen%shlibext -passes='JuliaFunctionSplitting,JuliaFunctionSplitting,verify' -julia-split-function-threshold=50 -julia-split-block-insts=40 -julia-split-region-insts=0 -S %s | FileCheck %s --check-prefix=IDEM
...
; instruction spacing alone (block-insts=200, larger than the whole function)
...
; Same rejection flow, but the sunk alloca's address computation has a
; non-constant index (%idx) defined in the entry block. Only the alloca may
; be hoisted back to the entry: re-anchoring the stranded GEP at the entry's
; front would lift it above %idx and break dominance. The GEP is legal IR
; where it was stranded once the hoisted alloca dominates it again.
; REJECT-LABEL: define i64 @rejected_region_dynidx(
; REJECT-NEXT: top:
; REJECT-NOT: getelementptr inbounds [2 x i64]
; REJECT: %slot = alloca [2 x i64]
; REJECT-NOT: getelementptr inbounds [2 x i64]
; REJECT: %idx = fptosi double %x to i64
; REJECT: getelementptr inbounds [2 x i64], ptr %slot, i64 0, i64 %idx

define i64 @rejected_region_dynidx(double %x, ptr addrspace(10) %obj) {
top:
  %pg = call ptr @julia.get_pgcstack()
  %idx = fptosi double %x to i64
  %slot = alloca [2 x i64], align 8
  %q = getelementptr inbounds [2 x i64], ptr %slot, i64 0, i64 %idx
  br label %work
