...
function construct_loopinfo(ir)
    cfg = ir.cfg
    @timeit "domtree 3" domtree = construct_domtree(cfg.blocks)
    # 1. find backedges
    ...
    for (n, header) in backedges
        # merge loops that have the same header
        if haskey(loops, header)
            visited = BitSet(loops[header].blocks)
            latches = loops[header].latches
        else
            visited = BitSet((header,))
            latches = Int[]
        end
        push!(visited, n)
        push!(latches, n)
        ...
        loops[header] = LoopInfo(header, latches, blocks)
    end
...
function licm_pass!(ir::IRCode, loops)
    ...
    for (h, loop) in loops
        ...
        insert_node!(ir, insertion_point, NewInstruction(GotoNode(h), Any), true)
        ...
        for pred in predecessors
            ...
            @show loop.header
            @show pre_header
            succs = zeros(Int, length(bb.succs))
            for (n,succ) in enumerate(bb.succs)
                succs[n] = succ == loop.header ? pre_header : succ
            end
            @show "here"
        ...
        for id in header.stmts
            ...
            stmt = node[:inst]
            if stmt isa PhiNode
                edges = Int32[bb == entry_bb ? pre_header : bb for bb in stmt.edges]
...
function invariant(ir, loop, invariant_stmts, stmt)
    ...
    if stmt isa Core.MethodInstance
        return decode_effects_override(stmt.def.purity).effect_free
    end
    return false
end
